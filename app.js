 ...
 * @constructor
 */
function GameBoard ({prop, children}) {
    let rows = listGenerator(0, 6);
    let singleRow = rows.map((row, k) => <div key={k} className={"row row-"+row}></div>)

    let columns = listGenerator(0, 9);
    return columns.map((column, k) => <div key={k} className={"columns columns-"+column+" flex-auto"}>{singleRow}</div>)
}
ReactDOM.render(<GameBoard></GameBoard>, document.querySelector('#board'))

function Snails ({children}) {
    let snails = listGenerator(1, 7);
    return snails.map((snailPos, k) => <div key={k} className={"snail-track"}>{SnailSvg(snailPos)}</div>)
}
ReactDOM.render(<Snails></Snails>, document.querySelector('#snail'))

function Dices ({diceLeft, diceRight, children}) {
    if (!diceLeft) {
        diceLeft = 0;
    }
    if (!diceRight) {
    ...
    return <div className={"dice d-flex justify-content-between"} onClick={RollDices}>
        <div className={"dice-edge"}>
            <div className={"dice-left dice-face dice-"+diceLeft}></div>
        </div>
        <div className={"dice-edge"}>
            <div className={"dice-right dice-face dice-"+diceRight}></div>
        </div>
    </div>
}
ReactDOM.render(<Dices></Dices>, document.querySelector('#dice'))

function RollDices (event) {
    let max = 6;
    let min = 1;

    let diceLeft = Math.floor(Math.random() * (max - min + 1) + min);
    let diceRight = Math.floor(Math.random() * (max - min + 1) + min);
    updateScore(diceLeft, diceRight);
    ReactDOM.render(<Dices diceLeft={diceLeft} diceRight={diceRight}></Dices>, document.querySelector('#dice'))
}

function SnailSvg(position) {

    return <svg version="1.1" id={"snail-img-"+position} className={"snail-img snail-img-"+position} style={{animationName: "slider-"+score['s'+position]}} xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" viewBox="0 0 511.996 511.996">
        <g>
            <g>
...
function updateScore(diceL, diceR) {
    if (score['s'+diceL] < 8 ) {
        score['s'+diceL] = score['s'+diceL] + 1;
    }

    if (score['s'+diceR] < 8) {
        score['s'+diceR] = score['s'+diceR] + 1;
    }
